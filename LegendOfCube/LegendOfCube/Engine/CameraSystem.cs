...
{	class CameraSystem
	{
		...
		private const float CAMERA_DISTANCE = 5.0f;
		private const float X_SCALE = 4.0f;
		private const float Y_SCALE = 4.0f;

		public void Initialize(World world)
		{
			world.CameraPosition = new Vector3(0.0f, 5.0f, 0.0f);
		}

		public void OnUpdate(World world, float delta)
		{
			ApproachPlayer(delta, world);
			ApplyCameraRotation(delta, world);
		}

		private void ApproachPlayer(float delta, World world)
		{
			...
			// Set camera to fixed distance from player
			Vector3 newTargetOffset = CAMERA_DISTANCE * Vector3.Normalize(cameraPosRelTarget);
			world.CameraPosition = cameraTarget + newTargetOffset;
		}

		private void ApplyCameraRotation(float delta, World world)
		{
